...
public class Ex01 extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		// 생성자와 멤버필드의 초기화, 객체 생성 시 한 번 호출된다.
...
/*		
		 서비스의 역할은 클라이언트가 정보를 요청할 때 요청 방식에 따라 알맞는 메소드를 호출하는 역할을 한다.
		 - 요청 방식
			-> get 방식이면 doGet()로 이동
			-> post 방식이면 doPost()로 이동
		 ** get방식(생략가능): 정보를 http 패킷의 head에 담아서 보낸다
							주소창에 해당 주소가 보인다.
							속도가 빠르다. 적은 양을 전달할 때 적합하며 보안에 취약하다.
 ...
 */	
	...
	// 서블릿이 자바코드와 HTML, CSS, JavaScript를 섞어 사용해서 웹페이지를 만드는 메소드(doGet(), doPost())
	...
	public void destroy() {
		// 해당 프로젝트 톰캣에서 삭제되기 직전에 실행하는 메소드
